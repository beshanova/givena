...
class ImportSql extends DB
{
	...
	function updateImagePath()    {
		$sql = "UPDATE cms_ru_module_c_catalog_items SET f_file = LOWER(f_file)";
		$this->mysqlquery($sql);

		$patch = S_ROOT . '/files/_import/';
		$handle = opendir($patch);
		while(($lfile = readdir($handle))) {
			if ( is_file ($patch."/".$lfile) )
			{
				rename ( $patch .  "/" . $lfile, $patch  . strtolower($lfile) );
			}
		}
    }
