...
class Catalog extends _List
{
    ...
    public $func='';
    public $tpl = '';
    const C_NAME = 'Catalog';
    const C_TITLE = 'Каталог';
    const C_DESC = 'Каталог';
    ...
    public function f_recomend_item($cnt=3)
    {
        $this->C_ISPUB = 0;
        $data = $this->_LIST->sql->getModuleData($this->tm_id);
        $catalog = $this->msql->getModuleItemsCondRecomend($cnt);
        return $this->ApplyTemplate($this->tpl, array('catalog'=>$catalog, 'data'=>$data, 'is_active'=>$data['topics_module_is_active']), $this->getClassName());
    }

    public function f_last_seen_item($cnt=3)
    {
        $this->C_ISPUB = 0;
        $data = $this->_LIST->sql->getModuleData($this->tm_id);
        $bask = new Basket();
        $catalog = $this->msql->getLastSeenItemsList($bask->getCurSessId(), $cnt);
        return $this->ApplyTemplate($this->tpl, array('catalog'=>$catalog, 'data'=>$data, 'is_active'=>$data['topics_module_is_active']), $this->getClassName());
    }

	  public function f_short_catalog_rec($cnt=3)
    {
        $this->C_ISPUB = 0;
        $data = $this->_LIST->sql->getModuleData($this->tm_id);
        $catalog = $this->_LIST->sql->getModuleItemsCondCatalog($this->tm_id, $cnt);
        return $this->ApplyTemplate($this->tpl, array('catalog'=>$catalog, 'data'=>$data, 'is_active'=>$data['topics_module_is_active']), $this->getClassName());
    }
