...
class BasketSql extends DB
{
    ...
    {
        $sql = 'SELECT * FROM ?_module_basket_items WHERE module_basket_item_type=?s AND (module_basket_item_session=?s) AND module_basket_item_status=0 ORDER BY module_basket_item_date_update DESC';
    ...
    function getBasketCntItemsOrders($type, $status=1)
    {
        $sql = 'SELECT COUNT(*) as CNT FROM ?_module_basket_items WHERE module_basket_item_type=?s AND module_basket_item_status=?i GROUP BY module_basket_item_status';
        $r = $this->query($sql, array($type, $status));
        return intval($r['CNT']);
    }

    function updateDopSumm($type, $oid, $summ)
    {
        $sql = 'UPDATE ?_module_basket_items SET module_basket_item_summ2="'.$summ.'" WHERE module_basket_item_type=?s AND module_basket_item_id=?i';
        $this->mysqlquery($sql, array($type, $oid));
    }

    function SearchOrderById($type, $id){
        $orders = array();
        $sql = 'SELECT * FROM ?_module_basket_items WHERE module_basket_item_type=?s AND module_basket_item_id=?i AND module_basket_item_status>0';
        $res = $this->mysqlquery($sql, array($type, $id));
    ...
    function getBasketItemsOrders($type, $status=1, $page, $cnt=10)
    {
        $orders = array();
        $sql = 'SELECT * FROM ?_module_basket_items WHERE module_basket_item_type=?s AND module_basket_item_status=?i ORDER BY module_basket_item_date_update DESC LIMIT ?i, ?i';
        $res = $this->mysqlquery($sql, array($type, $status, ($page-1)*$cnt, $cnt));
        while ($r = $this->fetchOne($res))
        {
            if ($r['module_basket_item_order']!='')
            {
    ...
    {
        $sql = 'UPDATE ?_module_basket_items SET module_basket_item_status=?i WHERE module_basket_item_id=?i';
        $this->mysqlquery($sql, array($status, $oid));
    }
    ...
    function updateBasketOrder($type, $data)
    {
        ...
        $o_text.= '<tr><td colspan="2"></td><td>Товаров: '.$order['cnt'].' шт.</td><td>Сумма: ' . $order['summ'] . '</td></tr>'.N;
        $o_text.= '</table>';
        $cnt = $order['cnt'];
        $summ = $order['summ'];
        ...
        $ins_arr[] = intval($_SESSION['_SITE_']['profiledata']['module_profile_item_id']);
        $ins_arr[] = $type;
        $ins_arr[] = $sess_id;
        $sql = 'UPDATE ?_module_basket_items SET module_basket_item_order=?s, module_basket_item_status=1, module_basket_item_date_update=NOW() '.$ins_txt.', module_basket_profile_item_id=?i WHERE module_basket_item_type=?s AND module_basket_item_session=?s AND module_basket_item_status=0';
    ...
    function getCurSessId()
    {
        if (!isset($_COOKIE['sess_id']))
        {
            ...
            setcookie('sess_id', $sess_id, time()+3600*24*365, '/');
        }
        else
    ...
    function add2Basket($type, $item_id, $cnt=1, $pr=1)
    {
        ...
        if (in_array($pr, array(1,2,3,4)))
        {
            ...
            if (!empty($q))
            {
                ...
                $sql = 'UPDATE ?_module_basket_items SET module_basket_profile_item_id=?i, module_basket_item_order=?s, module_basket_item_date_update=NOW() WHERE module_basket_item_type=?s AND module_basket_item_session=?s AND module_basket_item_status=0';
                $this->mysqlquery($sql, array($profile_id, $datas, $type, $sess_id));
            }
            else
            {
                $cat = new _List(ucfirst($type));
                ...
                $sql = 'INSERT INTO ?_module_basket_items SET module_basket_profile_item_id=?i, module_basket_item_order=?s, module_basket_item_type=?s, module_basket_item_session=?s, module_basket_item_date_update=NOW()';
                $this->mysqlquery($sql, array($profile_id, $datas, $type, $sess_id));
            }
        }
    ...
    function testOrderByProfile($ido, $idp)
    {
        $sql = 'SELECT * FROM ?_module_basket_items WHERE module_basket_item_id=?i AND module_basket_profile_item_id=?i';
        $q = $this->query($sql, array($ido, $idp));
