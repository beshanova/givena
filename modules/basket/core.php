...
class Basket extends ExtController
{
    ...
    private function actionPopup()
    {
        ...
        switch ($this->action)
        {
            ...
            case 'searchbynum':
                $searchid = intval($_REQUEST['searchid']);
                $status = $this->sql->getStatusesList();
                $data = $this->sql->SearchOrderById($this->tm_id, $searchid);
                print $this->ApplyTemplateAdmin('table_order.php', array('data'=>$data, 'status'=>$status), $this->getClassName());
                exit;
