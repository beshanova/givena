...
class ContentSql extends DB
{
    private $ok_tags = array('<br>', '<p>', '<span>', '<a>', '<li>', '<ol>', '<ul>', '<hr>', '<em>', '<u>', '<strike>', '<strong>', '<b>', '<i>', '<del>', '<noindex>', '<table>', '<tr>', '<td>', '<tbody>', '<thead>', '<caption>', '<th>');
    ...
    {
        $sql = 'SELECT * FROM ?_module_content, ?_topics_modules WHERE module_content_topic_modules_id=?i AND topics_module_id=module_content_topic_modules_id';
    ...
    function saveContentData($id)
    {
        ...
        $content = strip_tags($_REQUEST['content'], implode('', $this->ok_tags));
        $content = preg_replace('~\<([^(a|table)]{1}[a-z]{0,8})\s.+\>~isUm', '<$1>', $content);
        $content = preg_replace('~('.implode('|', $this->ok_tags).'){2,}~isUm', '$1', $content);

        $sql = 'UPDATE ?_module_content SET module_content_text=?s, module_content_tpl=?s, module_content_date_update=NOW() WHERE module_content_topic_modules_id=?i';
    ...
    function saveNewBlock($id)
    {
        ...
        $sql = 'INSERT INTO ?_module_content SET module_content_topic_modules_id=?i, module_content_tpl=?s, module_content_date_update=NOW()';
        $this->mysqlquery($sql, array( $id, $_REQUEST['tpl'] ));
    }

    function deleteContentBlock($id)
    {
        $sql = 'DELETE FROM ?_module_content WHERE module_content_topic_modules_id=?i';
