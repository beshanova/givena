...
class BasketSql extends DB
{
    ...
    function getBasketItemsData($type, $sess_id)
    {
    ...
    function updateBasketOrder($type, $data)
    {
        ...
        foreach ($data['cnt'] as $pid=>$cnt)
        {
            ...
            foreach ($order['items'][$pid] as $k=>$i)
                if (preg_match('~^f_~i', $k) && $k!='f_price' && !is_array($i))
                    $o_text .= $i.", ";
            $o_text .= "(".$order['items'][$pid]['f_price']." руб., ".$cnt." шт.)".N;
        }
        ...
        $text = "На сайте ".S_HOST." оформлен заказ №".$n_order.N.N;
        $text .= "Информация о заказчике:".N.$c_text.N;
        $text .= "Информация по заказу:".N.$o_text;
    ...
    function add2Basket($type, $item_id, $cnt=1)
    {
        $sess_id = $this->getCurSessId();
        ...
        if (!empty($q))
        {
            ...
            if (!isset($datas[$item_id]))
            {
                $cat = new _List(ucfirst($type));
                $datas[$item_id] = $cat->add2BasketItem($item_id);
            }
            $datas[$item_id]['cnt'] += $cnt;
            $datas[$item_id]['url'] = urldecode($_REQUEST['bu']);
    ...
    private function getFieldType($r, $value="")
    {
        ...
        switch ($type)
        {
            ...
            case 'select':
                if (preg_match_all("~'(.+)'~isU", $r['COLUMN_TYPE'], $ar))
                {
                    $data['field'][] = '<select id="b-' . $name . '" class="admin-field-style-select" name="' . $name . '">';
