...
class Advanced_search extends ExtController
{
    ...
    public function __construct()
    {
        global $APP;
        $this->app = $APP;
        $this->sql = new Advanced_searchSql();
    ...
    private function getContent()
    {
        if ($this->action=='adv_search_res')
        {
            ...
            $data['query'] = trim($_REQUEST['color']);
            ...
            $min_h = intval($_REQUEST['height_from']);
            $max_h = intval($_REQUEST['height_to']);

            if ((sizeof($q_r)>0)||($min_h)||($max_h))
            {
                $data['results'] = $this->sql->searchTextInModules($q_r, $min_h, $max_h, $data['module_search_cnt_literals']);
