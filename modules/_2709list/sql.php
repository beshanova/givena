...
class _ListSql extends DB
{
    ...
    function getModuleItems($id, $cnt=10, $page=1)
    {
        $datas = array();
        if ($_SESSION['_SITE_']['is_adm']==1)
            $sql = 'SELECT I.* FROM ?_module_cz_relations_items R, ?_module_c_'.($this->cl).'_items I WHERE R.module_rel_module_id=?i AND R.module_rel_item=I.module_list_item_id ORDER BY R.module_rel_sortby LIMIT ?i, ?i';
        else
            $sql = 'SELECT I.* FROM ?_module_cz_relations_items R, ?_module_c_'.($this->cl).'_items I WHERE R.module_rel_module_id=?i AND R.module_rel_item=I.module_list_item_id AND module_list_item_is_active=1 ORDER BY R.module_rel_sortby LIMIT ?i, ?i';
...
/* 	function getModuleItemsCondCatalog($id, $cnt)
    ...
    public function searchContentModule($tm_ids, $q_arr)
    {
        ...
        foreach ($fields as $f)
        {
            if (in_array($f['f_type'], array('select','text', 'textarea', 'file')))
