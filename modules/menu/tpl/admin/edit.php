...
<table class="menu_edit">
  ...
  <tr id="menu-item-<?= $m['module_menu_id'] ?>" mi="<?= $m['module_menu_id'] ?>" parent="<?= $m['module_menu_parent_id'] ?>"<?= ($m['level']>0 ? ' style="display:none;"' : '') ?> class="menu-item-00 <?= $tr_cl ?>" level="<?= $m['level'] ?>">
		<td style="padding-left:<?= ($m['level']*30 + 20) ?>px;"><?= ($m['is_childs'] ? '<div class="menu_edit_close" onclick="hide_show_parent(this);"></div>' : '') ?>
      ...
      <input type="hidden" name="menu[<?= $m['module_menu_id'] ?>][title]" value="<?= htmlspecialchars($m['module_menu_title']) ?>" />
		...
		<td>
			<div class="menu_edit_edit tooltip" onclick="f_edit_title(this);"><span class="tooltip-span">Нажмите на иконку, чтобы </br> изменить название раздела</span></div>
		</td>
		<td> </td>
		<td>
      ...
      <input type="hidden" name="menu[<?= $m['module_menu_id'] ?>][url_parent_hidden]" value="<?= str_replace($m['module_menu_url'], '', $m['urls']) ?>" />
		...
		<td> </td>
		<td>
			<div class="tooltip <?= ($m['is_last_level'] ? 'menu_edit_sort_down_not_activ' : 'menu_edit_sort_down') ?>" onclick="f_edit_sort_down(this);"><span class="tooltip-span">Нажмите на иконку, чтобы </br> переместить ниже</span></div>
			<div class="tooltip <?= ($k!=1 ? 'menu_edit_sort_up' : 'menu_edit_sort_up_not_activ') ?>" onclick="f_edit_sort_up(this);"><span class="tooltip-span">Нажмите на иконку, чтобы </br> переместить выше</span></div>
      <input type="hidden" name="menu[<?= $m['module_menu_id'] ?>][sort]" value="<?= $m['module_menu_sortby'] ?>" id="menu-item-b-sort-<?= $m['module_menu_id'] ?>" />

			<div class="tooltip <?= ($k==1 || $m['level']>$pm['level'] ? 'menu_edit_sort_dr_not_activ' : 'menu_edit_sort_dr') ?>" onclick="f_edit_parent(this);"><span class="tooltip-span">Нажмите на иконку, чтобы </br> закрепить за родителем</span></div>
      ...
      <input type="hidden" name="menu[<?= $m['module_menu_id'] ?>][level]" value="<?= $m['level'] ?>" id="menu-item-level-<?= $m['module_menu_id'] ?>" />
		...
		<td>
			<div class="menu_edit_hide tooltip" onclick="f_edit_hide(this);"><span class="tooltip-span">Нажмите на иконку, чтобы </br> скрыть</span></div>
    ...
    <? else : ?>
      <div class="menu_edit_delete tooltip" onclick="f_edit_delete(this);"><span class="tooltip-span">Нажмите на иконку, чтобы </br> удалить</span></div>
  ...
  <tr id="menu-item-xx" style="display:none;">
		<td style="padding-left:20px;">
			<input value="" name="menu[xx][title]" onkeyup="f_translit_url($(this).val(), 'xx');">
		</td>
		<td> </td>
		<td> </td>
		<td>
      <input type="text" name="menu[xx][url]" value="" id="menu-url-xx" />
    </td>
		<td> </td>
		<td>
...
<br>
<input type="button" value="Добавить раздел" class="admin-popup-window-button-save" style="float: left;" onclick="f_add_new_tr();">
<div class="legend-menu">
	<div class="legend-menu-link"><span></span> - ссылка</div>
	<div class="legend-menu-hide"><span></span> - скрытый</div>
	<div class="legend-menu-del"><span></span> - удаленный</div>
	<div class="legend-menu-new"><span></span> - новый</div>
</div>

<br><br><br>

<input type="button" value="Применить" class="admin-popup-window-button-save" onclick="f_save_menu();" />
<input type="submit" value="Применить и закрыть" class="admin-popup-window-button-save" />
...
jQuery(".tooltip").hover
    ...
    function ()
    {
      jQuery(this).find('.tooltip-span').fadeIn(2);
    },
    function ()
    {
      jQuery(this).find('.tooltip-span').fadeOut(0);
    }
	);
...
$(document).ready(function(){
  ...
  f_translit_url = function(w, g_m_id)
  {
    w=w.toLowerCase();
    ...
    var trans = ww.replace(/[^a-zA-Z0-9\-]/g,'-').replace(/[-]{2,}/gim, '-').replace( /^\-+/g, '').replace( /\-+$/g, '');
    $('#menu-url-'+g_m_id).val( (trans ? '/'+trans+'/' : ''));
  }

  f_save_menu = function()
  {
    var data = { 'admin_action':'save_w', 'ajax':1, cl:'<?= $cl ?>', tm:'<?= $tm_id ?>' };
    $('table.menu_edit input').each(function(){
    ...
    $.ajax({
        ...
        $('#draggable').html(data);
		  ...
		  beforeSend: function()
		  {
			  $('div.admin-popup-window-form').html('Идет сохранение...');
		  }
		});
		return false;
  }
  ...
  f_add_new_tr = function()
  {
    ...
    $('#menu-item-xx').before('<tr id="menu-item-'+glob_max_id+'" mi="'+glob_max_id+'" parent="0" class="menu-item-00 menu_edit_new_item" level="0">' + tr_text.replace(/xx/ig, glob_max_id) + '</tr>');
